...
function createEventEmitter<T extends Record<string, any>>() {
  ...
  return {
    ...
    emit<K extends keyof T>(eventName: K, data: T[K]) {
      // In a server environment, we might want to log this to a different place.
      if (typeof window === 'undefined') {
        console.error('SERVER-SIDE PERMISSION ERROR:', JSON.stringify((data as FirestorePermissionError).request, null, 2));
        return;
      }
      if (!events[eventName]) {
        return;
