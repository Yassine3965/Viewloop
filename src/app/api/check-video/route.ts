...
export async function POST(req: Request) {
  ...
  if (!verifySignature(req, body)) {
    const response = NextResponse.json({ error: "INVALID_SIGNATURE", exists: false }, { status: 403 });
    return addCorsHeaders(response, req);
  }
  ...
  try {
    ...
    if (!videoId) {
      const response = NextResponse.json({ error: 'Video ID required', exists: false }, { status: 400 });
      return addCorsHeaders(response, req);
    }
    ...
    if (exists) {
      ...
      const response = NextResponse.json({
        ...
        message: 'Video found in ViewLoop database'
      });
      return addCorsHeaders(response, req);
    } else {
      const response = NextResponse.json({
    ...
    const response = NextResponse.json(
      { error: 'Internal server error', details: error.message, exists: false },
      { status: 500 }
    );
    return addCorsHeaders(response, req);
  }
